...
def train(train_id, n_colors, data):
    ...
    test_gnn_mean_score = (gnn_predictions[~training_mask]).float().mean().item()
    test_gnn_var_score = (gnn_predictions[~training_mask]).float().var().item()
    test_att_mean_score = (att_predictions[~training_colors]).float().mean().item()
...
def main():
    ...
    edge_index, color_indices, labels = load_vae_extracted_data()

    # Find n_colors by the length of the labels
    n_colors = len(labels)

    # Set a results df
    results_df = pd.DataFrame(columns=["test_labels", "train_gnn_accuracy", "train_gnn_auc",
    ...
    for i in tqdm(range_test_colors, desc="Training for each color", total=n_colors):
        ...
        # results_df.to_csv(join(RESULTS_DIR, "results_vae", f"results_vae_first_model.csv"), index=True)
        results_df.to_csv(join("results_vae_first_model.csv"), index=True)
