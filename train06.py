...
def main():
    ...
    # Find train colors
    train_colors = color_indices[~test_nodes].unique()
    # Find their labels
    train_labels = labels[train_colors]
    # Get the portions of the labels
    positives_portion = train_labels.sum() / len(train_labels)
    negatives_portion = 1 - positives_portion
    print(f"Portion of positives: {positives_portion:.2f}", f"Portion of negatives: {negatives_portion:.2f}")

    ## Subsample the training set, with *minimal* undersampling of the majority class, to balance the classes
    # Find the majority and minority colors
    if negatives_portion > 0.6:
        majority_color = train_colors[train_labels == 0]
        minority_color = train_colors[train_labels == 1]
    else:
        majority_color = train_colors[train_labels == 1]
        minority_color = train_colors[train_labels == 0]
    # Subsample the majority colors to be the same size as the minority colors
    majority_sample_size = len(minority_color)
    majority_permuted = majority_color[torch.randperm(len(majority_color))]
    majority_sampled = majority_permuted[:majority_sample_size]
    majority_unsampled = majority_permuted[majority_sample_size:]
    # Concatenate the minority colors and the sampled majority colors to create a balanced training set
    balanced_train_colors = torch.cat([minority_color, majority_sampled])

    # Create a balanced edge index that only contains edges between the balanced training colors
    balanced_train_mask = torch.isin(color_indices, balanced_train_colors)
    balanced_edge_mask = balanced_train_mask[edge_index[0]] & balanced_train_mask[edge_index[1]]
    balanced_train_edge_index = edge_index[:, balanced_edge_mask]

    # Concat the test_colors with the unsampled majority colors for the training stage
    test_and_unsampled = torch.cat([test_colors, majority_unsampled])
    ...
    for test_color in tqdm(test_colors, desc="Iterating through test colors", total=len(test_colors)):
        ...
        temp_edge_index = torch.cat([balanced_train_edge_index, test_edge_index], dim=1)

        # Define test labels
        # Train the model with the current test color
        _, _, _, result_dct = train(n_colors, data=(temp_edge_index, color_indices,
                                                    labels, test_colors), tested_color=test_and_unsampled)
